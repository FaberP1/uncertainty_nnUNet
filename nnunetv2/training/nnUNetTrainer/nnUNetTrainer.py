...
class RenameTargetKeyTransform(AbstractTransform):
    """Renames 'target' to 'seg'."""
    def __call__(self, **data_dict):
        data_dict['seg'] = data_dict.pop('target')
        return data_dict


class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), num_epochs:int = 1000,  num_of_cycles: int = 1, gamma: float = 0.8 ,rule:str = 'sparse', save_last = 2, alpha_r = 0.01): #$ change 1
        ...
        ### Some hyperparameters for you to fiddle with #$ change2
        self.initial_lr = 10 * alpha_r if num_of_cycles > 1 else alpha_r
        ...
        self.save_every = 50
        self.save_last = save_last #$ change3
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            print(f'Trainer: 238: self.loss is {self.loss}')
    ...
    def _build_loss(self):
            if self.label_manager.has_regions:
                ...
                print(f'trainer 383: dc&ce loss used')
    ...
    def configure_rotation_dummyDA_mirroring_and_inital_patch_size(self):
        ...
        print('lets see, patch size: ', patch_size) #debugging line
        dim = len(patch_size)
        ...
        initial_patch_size = get_patch_size(patch_size[-dim:],
                                            *rotation_for_DA.values(),
                                            (0.85, 1.25))
        initial_patch_size = patch_size #faber addition
        if do_dummy_2d_data_aug:
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = PolyLRScheduler(optimizer, self.initial_lr, self.num_epochs, self.num_of_cycles, self.gamma, self.exponent, alpha_r = self.alpha_r) #$ change4
    ...
    def get_dataloaders(self):
        ...
        print('lets see, initial patch size = ', initial_patch_size)
    ...
    def get_training_transforms(
        ...
        tr_transforms = []
        tr_transforms.append(RenameTargetKeyTransform())
    ...
    def get_validation_transforms(
        ...
        val_transforms = []
        val_transforms.append(RenameTargetKeyTransform())
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        #print("target shape within train_step:", target[0].shape)
    ...
    def on_epoch_end(self):
        ...
        #$ change5 handling cyclic checkpoints
        
        Tc = int(self.num_epochs/self.num_of_cycles)
        ...
        if self.rule == 'late':
            ...
            # $ save early checkpoint: now saves equal amount as when saving the last x models
            early_saving_threshold = np.linspace(int(Tc * 0.8) , Tc - 1, self.save_last, dtype=int).tolist()
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            epoch_numbers += 1
            print(f'about to start on epoch {epoch_numbers}...')    
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                train_steps += 1
                print(f'about to start train_step {train_steps}...')    
                # Check the batch content and structure
            ...
            self.on_epoch_end()
            print(f'done with epoch {epoch_numbers}!')
