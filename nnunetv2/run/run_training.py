...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
                          ...
                          gamma:float = 0.8,
                          rule = 'sparse',
                          alpha_r = 0.01): #$ change1 added num_of_cycles, epochs and gamma):
    ...
    #$ change2 make sure that num_epochs/num_of_cycles produces an integer
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                    ...
                                    num_epochs = num_epochs,  num_of_cycles = num_of_cycles, gamma = gamma, rule = rule, alpha_r = alpha_r)  #$ change3 added num_of_cycles, epochs and gamma)
...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
    ...
    if continue_training:
        ...
        if not isfile(expected_checkpoint_file):
            expected_checkpoint_file = join(nnunet_trainer.output_folder, 'e_checkpoint_398.pth')
        #$ added this for a failed training case :(
        if not isfile(expected_checkpoint_file):
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, use_compressed, disable_checkpointing, c, val,
            pretrained_weights, npz, val_with_best, world_size, num_epochs, num_of_cycles, gamma): #$ change4 added num_of_cycles, epochs and gamma
    setup_ddp(rank, world_size)
    torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, tr, p,
                                           use_compressed, num_epochs, num_of_cycles, gamma, rule, alpha_r) #$ change5 added num_of_cycles, epochs and gamma
...
def run_training(dataset_name_or_id: Union[str, int],
                 ...
                 gamma:float = 0.8,
                 rule:str = 'sparse',
                 alpha_r: float = 0.01): #$ change6 added num_epochs, num_of_cycles and gamma):
    ...
    if num_gpus > 1:
        ...
        mp.spawn(run_ddp,
                 args=(
                     ...
                     gamma,
                     rule,
                     alpha_r), #$ change7 added num_epochs, num_of_cycles and gamma
                 nprocs=num_gpus,
                 join=True)
    else:
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, trainer_class_name,
                                               plans_identifier, use_compressed_data, device=device,
                                               num_epochs= num_epochs , num_of_cycles = num_of_cycles, gamma = gamma, rule = rule, alpha_r = alpha_r) #$ change8 added num_epochs, num_of_cycles and gamma
...
def run_training_entry():
    ...
    parser.add_argument('-gamma', type=float, default=0.8, required=False,
                    help="Gamma value for cycle stage 2 - the learning rate scheduler will keep constant lr after [gamma]*total_epochs first epochs")
    parser.add_argument('-rule', type=str, default='sparse', required=False,
                    help="How do you want the chckpoint saving at each cycle? possible are [late,sparse,both,none] \n late - saves the 2 last at each cycle. \n sparse - saves 2 checkpoints starting at 0.7*Tc. \n both - saves both")
    # parser.add_argument('-save_models', type=int, default=3, required=False,
    #                 help="How many models do you want to save from each cycle?")
    parser.add_argument('-alphar', type=float, default=0.01, required=False,
                    help="What do you want the learning rate to be at the start of a new epoch cycle? the shock step will be 10 times this value. For one cycle, alpha_r is the starting learning rate.")
    args = parser.parse_args() #$ change8 added all these arguments
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, num_epochs=args.num_epochs, num_of_cycles=args.num_of_cycles, gamma=args.gamma , rule=args.rule, alpha_r=args.alphar)
