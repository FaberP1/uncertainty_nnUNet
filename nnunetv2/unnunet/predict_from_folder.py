...
def predict_from_folder(dataset, fold:int , indir , outdir , rule = 'both'):
    ...
    if rule == 'late':
        # take only checkpoints that start with 'checkpoint_'
        checkpoint_list = [checkpoint for checkpoint in checkpoint_list if checkpoint.startswith('checkpoint_')]

    elif rule == 'sparse':
        # take only checkpoints that start with 'e_checkpoint_'
        ...
        raise ValueError("No extra checkpoints were saved, use normal nnunet_predict to run inference")

    else:
        # raise an error for any invalid rule
        raise ValueError(f"Invalid rule '{rule}', expected one of ['late', 'sparse', 'both', 'none']")
    ...
    for checkpoint in checkpoint_list:  
        ...
        checkpoint_output_folder = outdir  + '/' + str(checkpoint).replace('.pth','') + f'_fold{fold}'
