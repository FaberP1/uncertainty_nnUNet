...
def entropy_map_fun(bg_map, class1_map, class2_map, class3_map):
    # Stack all class maps along a new dimension
    combined_map = np.stack([bg_map, class1_map, class2_map, class3_map], axis=0)
    
    # Add a small epsilon to avoid log2(0) which results in -inf
    epsilon = 1e-10
    combined_map = np.clip(combined_map, epsilon, 1.0)
    
    # Calculate the entropy across the first dimension
    entropy_map = -np.sum(combined_map * np.log2(combined_map), axis=0)
...
def T_test_on_single_image(
    background_array: np.ndarray,
    class1_array: np.ndarray, #tumor
    class2_array: np.ndarray, #dhgp
    class3_array: np.ndarray, #liver
    title: str = 'p_values_map',
    plot_results: bool = False
):
    # Perform t-tests between background and each foreground class
    results = {}
    for i, class_array in enumerate([class1_array, class2_array, class3_array], start=1):
        # Perform t-test with unequal variances
        t_values, p_values_map = stats.ttest_ind(background_array, class_array, axis=0, equal_var=False)
        
        # Store the p-value map in the results dictionary
        results[f'class_{i}'] = p_values_map.T
        
        # Plot the p-value map if requested
        if plot_results:
            fig, ax = plt.subplots(1, 1, figsize=(10, 10))
            ax.imshow(p_values_map, cmap='viridis')
            ax.set_title(f"{title} - Class {i}")
            plt.show()
...
def uncertainty_from_mask_and_valmap(p_values_map: np.ndarray, mask: np.ndarray, plot: bool = False, image: np.ndarray = None):
    ...
    if plot:
        ...
        ax[0].set_title('P-values map + edges, score: ' + str(certainty_score))
