...
class LabelManager(object):
    ...
    def downsample_probabilities(self, prob_array: Union[torch.Tensor, np.ndarray], factor=16):
        ...
        if prob_array.shape[1] == 1:
            prob_array = prob_array.squeeze(1)  # Squeeze the singleton dimension, resulting in shape (c, x, y)
        
        # Get the current shape of the array
        c, x, y = prob_array.shape
        
        # Check if x and y are divisible by the factor
        if x % factor != 0 or y % factor != 0:
            raise ValueError("The spatial dimensions of the array must be divisible by the downsample factor.")
        
        # Reshape and take the mean over blocks of (factor, factor)
        prob_array_downsampled = prob_array.reshape(c, x // factor, factor, y // factor, factor).mean(axis=(2, 4))
        
        return prob_array_downsampled
